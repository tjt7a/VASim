...
int main(int argc, char *argv[])
{
    ...
    for (Automata *a : merged)
    {
        ...
        if (to_hls)
        {
            int split_factor = 5;
            int CONFIG_SIZE = 23;
            a->automataToHLSFiles(num_automata, split_factor, CONFIG_SIZE);
        }
        ...
        {
            if (!quiet)
                cout << "Emitting automata as .blif circuit..." << endl
                     << endl;
        ...
        {
            if (!quiet)
                cout << "Emitting automata in .graph format for HyperScan ingestion..." << endl
                     << endl;
